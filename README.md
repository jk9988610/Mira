...
| 系统 | 说明 |
|------|------|
| **订单** | 族长根据周围需求发布单一订单；化身者接单后前往坐标展开光环履约 |
| **化身** | Q 农场 · Z 校园 · X 乐园 · C 堡垒（堡垒需保卫者登记） |
| **光环** | 农场/校园/乐园/堡垒/地图地块均通过光环传递效果；HUD 显示当前所受光环 |
| **压力** | 家族人数超阈值后释放隐藏压力；过高则被他族敌视，敌族在堡垒光环下受伤 |
| **地块** | 淡绿食物 · 淡紫知识 · 淡蓝快乐 |
| **视角** | `V` 切换观察任意圆（不可操控被观察者） |

按键：`Q` 农场 · `E` 生产 · `Z` 校园 · `X` 乐园 · `C` 堡垒 · `V` 切换视角 · `P` 暂停
