...
2D 圆形吞噬游戏 — **触屏优先**，面向安卓平板横屏；手柄/键盘为辅助输入。

> **当前阶段：家族模拟观察者模式** — 无玩家实体，触屏拖拽平移视角；全屏统计暂停世界

详细设计见 [圆的化身系统](docs/AVATAR_SYSTEM.md)。
...
主菜单 → **开始游戏** 进入化身模式（详见 [docs/AVATAR_SYSTEM.md](docs/AVATAR_SYSTEM.md)）。

| 系统 | 说明 |
|------|------|
| **订单** | 族长发布单一需求订单；化身者/保卫者前往坐标完成服务即结单 |
| **堡垒订单** | 成员受敌族压力时族长发单；仅本族男性保卫者可接，化堡 8 秒 |
| **压力** | 每圆压力=质量；家族总压力超阈值被他族敌视，低于阈值取消敌视 |
| **操控** | 虚拟键 A农场 B校园 X乐园 Y堡垒；LB/RB切视角，LB+RB回自己 |

### 竞技模式（历史规划，当前菜单未入口）

- 单局 **6 分钟**，本地 **20 人**竞技
- **Q 分身** / **E 聚集** 等机制见 [docs/OUTLINE.md](docs/OUTLINE.md)
...
| 文档 | 作用 |
|------|------|
| **[docs/AVATAR_SYSTEM.md](docs/AVATAR_SYSTEM.md)** | 化身模式：订单、光环、堡垒、压力、地块、视角 |
| **[docs/OUTLINE.md](docs/OUTLINE.md)** | 竞技模式详细设计：玩法、物理公式、AI、相机 |
